...
int bon(int temp[],int at[],int p[],int arv,int n)
{
	int a=0,count=0;
	for(int i=1;i<=n;i++)
	{
...
int main()
{
	...
	for(int i=1;i<=n;i++)
	{
		q[i]=bon(temp,temparv,p,ct[q[i-1]],n);
...
wt[q[i]]=(ct[q[i-1]]>at[q[i]])?(ct[q[i-1]]-at[q[i]]):0;
	...
	for(int i=1;i<=n;i++)
	{
		cout<<q[i]<<"   "<<at[q[i]]<<"    "<<bt[q[i]]<<"    "<<ct[q[i]]<<"    "<<tat[q[i]]<<"    "<<wt[q[i]]<<"    "<<endl;
	}
